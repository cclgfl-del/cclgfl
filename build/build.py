...
def load_config():
    ...
    cfg["categories_by_slug"] = OrderedDict((c["slug"], c) for c in cfg["categories"])
    return cfg


# ── content ───────────────────────────────────────────────────────────────

def load_folder(path, cats, **kw):
    posts = []
    if not path.exists():
        return posts
    for folder in sorted(p for p in path.iterdir() if p.is_dir() and (p / "index.md").exists()):
        posts.append(load_post(folder, cats, **kw))
    return posts


def load_all(cfg, with_specimens):
    cats = cfg["categories_by_slug"]
    posts = load_folder(ROOT / "content" / "posts", cats)
    if with_specimens:
        posts += load_folder(ROOT / "content" / "specimens", cats)
    previews = []
    pdir = ROOT / "content" / "previews"
    if pdir.exists():
        for folder in sorted(p for p in pdir.iterdir() if p.is_dir() and (p / "index.md").exists()):
            previews.append(load_post(folder, cats, preview_token=folder.name))
...
def make_env(cfg, posts, specimens_build):
    ...
    counts = {c: 0 for c in cfg["categories_by_slug"]}
    for p in posts:
        counts[p.category["slug"]] += 1

    env.globals.update(
        cfg=cfg, url=url, abs_url=abs_url, asset=asset, pattern=pattern,
        categories=list(cfg["categories_by_slug"].values()), category_counts=counts,
...
def read_next(post, posts, n=3):
    same = [p for p in posts if p is not post and p.category is post.category]
    rest = [p for p in posts if p is not post and p.category is not post.category]
    return (same + rest)[:n]
...
def build_pages(env, cfg, posts, previews):
    ...
    for cat in cfg["categories"]:
        in_cat = [p for p in posts if p.category is cat]
        write(OUT / "category" / cat["slug"] / "index.html",
              render("category.html", page="category", path="/category/%s/" % cat["slug"],
                     category=cat, posts=in_cat))

    years = OrderedDict()
    for p in posts:
        years.setdefault(p.date.year, []).append(p)
    write(OUT / "posts" / "index.html",
          render("posts.html", page="posts", path="/posts/", years=years, total=len(posts)))

    write(OUT / "about" / "index.html", render("about.html", page="about", path="/about/"))
    write(OUT / "submit" / "index.html", render("submit.html", page="submit", path="/submit/"))
    write(OUT / "search" / "index.html", render("search.html", page="search", path="/search/"))
...
def build_feed(cfg, posts):
    ...
    for p in posts[:30]:
        ...
        items.append(
            ...
            "<pubDate>%s</pubDate><category>%s</category>%s<description>%s</description></item>" % (
                xml_escape(p.title), link, link, rfc822(p.date), xml_escape(p.category["name"]),
...
def build_sitemap(cfg, posts):
    paths = ["/", "/posts/", "/about/", "/submit/"] + ["/category/%s/" % c for c in cfg["categories_by_slug"]]
...
def build_patterns(cfg):
    ...
    for slug in cfg["categories_by_slug"]:
        guilloche.write_category_patterns(slug, out)
